...
【要素9: alternatives_view】
オルタナティブ資産クラス（プライベート・エクイティ、プライベート・デット、不動産、インフラ）について、
**初心者向けに丁寧に**当日〜直近のニュースを取りまとめてコメントする。
- 専門用語は出てきたタイミングで簡潔に補足する（例: "PEファンドの『ドライパウダー』(投資未実行の資金)"）
- 4資産クラスすべてを毎日カバーする必要はないが、ニュースで動きがあったクラスは取り上げる
- 「なぜ初心者にとって重要か」「市場全体や上場資産にどう波及しうるか」の視点を織り込む
- 著名ファンド（Blackstone, KKR, Carlyle, Brookfield 等）や巨額ディール、規制動向、業界トレンドが入りやすい
- 過度に強気・弱気を断定せず、観察と論点提示のトーン

返す内容:
  - body  : 解説本文 (300〜500字)。複数段落OK。専門用語は親切に解説。
