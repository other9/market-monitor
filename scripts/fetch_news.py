"""
fetch_news.py  (v13.3 — 4-call split + common.py 利用)

RSSから直近ニュースを集め、Claude API を **役割別に 4 回** 呼び出して以下を生成:

  Call 1 (Opus 4.7) : news_and_charts
    1. epigraph             : 冒頭引用
    2. headline_of_the_day  : 本日の一言サマリ
    3. news (7本)           : 市場を動かしたニュース + ソースURL
    4. charts_of_the_day    : 注目チャートの候補 (5〜8本、優先度順)

  Call 2 (Opus 4.7) : deep_dive
    5. deep_dive            : 今日の深掘り記事
                              - 平日: 当日最重要ニュースの解説
                              - 土曜: 週次総括
                              - 月初2日間: 前月総括

  Call 3 (Sonnet 4.6) : cb_and_alts
    6. central_bank_watch (4本)
    7. pe_pd_view
    8. real_assets_view

  Call 4 (Haiku 4.5) : muse_and_economic
    9. funny_stories (3本)
   10. economic_chart_of_the_day

各呼び出しは独立して失敗を許容する: 1つが失敗しても他の要素は生成される。
data/news.json に書き出す。

歴史:
  v7  : 1 回の Opus 呼び出しで 10 要素を一括生成 (〜v13.1.x まで使用)
  v13.2: 4 分割。失敗の独立性 + Haiku/Sonnet 利用でコスト効率向上
  v13.3: scripts/common.py の log_* / JST / utc_now_iso に乗せ替え
"""
...
# v13.2: 役割別モデル選択
MODEL_OPUS   = "claude-opus-4-7"     # 最高品質: Deep Dive + News+Charts
MODEL_SONNET = "claude-sonnet-4-6"   # 中級品質: 中銀+オルタナ解説
MODEL_HAIKU  = "claude-haiku-4-5"    # 軽量: 小話+経済指標選定
...
def determine_cadence() -> dict[str, Any]:
    """週末/月初判定。JST基準 (v13.3: common.JST 利用)。"""
    now_jst = jst_now()
...
def fetch_rss_items(max_per_feed: int = 25, hours_window: int = 36) -> list[dict[str, str]]:
    ...
    for feed in RSS_FEEDS:
        try:
            ...
            log_warn(f"RSS fail: {feed['name']} {e}")
            continue
    ...
    log_info(f"Collected {len(items)} items from {len(RSS_FEEDS)} feeds")
    return items
...
def build_news_list(items: list[dict[str, str]], max_items: int = 150) -> str:
    """各 Claude 呼び出しに渡す共通のニュース一覧テキスト ([N] インデックス付き)。"""
    lines = []
...
# ============================================================
# Generic Claude caller (4 calls 共通の入出力処理)
# ============================================================
...
def _call_claude(
    ...
    *,
    ...
    """共通のメッセージ作成 + JSON パース。失敗時は raise (呼び出し側で catch)。"""
    ...
    try:
        ...
        log_warn(f"[{label}] Failed to parse Claude response as JSON: {e}")
        log_warn(f"[{label}] Raw text:\n{text[:1500]}")
        raise

    log_ok(f"[{label}] {model}: parsed {len(json.dumps(result))} bytes")
    return result


# ============================================================
# Call 1: news + charts (Opus 4.7)
# ============================================================

def build_news_and_charts_prompt() -> str:
    universe_prompt = chart_prompt_list()
    return f"""あなたは金融市場のアナリスト兼エディターです。
直近24時間のニュース見出しを元に、マーケット日報の以下4要素を日本語で作成します。

ニュース一覧はインデックス番号 [N] 付きで提供されます。news では参照した元記事の
インデックス番号を source_index で返してください（元記事URLの紐付けに使います）。
...
【要素4: charts_of_the_day (5〜8本、優先度順)】
...
def call_news_and_charts(
    client: Anthropic, items: list[dict[str, str]]
) -> dict[str, Any]:
    user_msg = (
        f"以下は直近24時間に配信された市場関連ニュースの見出し一覧です "
        f"（計{len(items)}件）。先頭の [N] がインデックス番号です。"
        f"システムプロンプトに従って、epigraph / headline_of_the_day / news / charts_of_the_day "
        f"の4要素のみを JSON で返してください。\n\n"
        + build_news_list(items)
    )
    return _call_claude(
        client,
        model=MODEL_OPUS,
        max_tokens=3500,
        system=build_news_and_charts_prompt(),
        user_msg=user_msg,
        label="news+charts",
    )
...
# ============================================================
# Call 2: deep_dive (Opus 4.7)
# ============================================================

def _build_deep_dive_instructions(cadence: dict[str, Any]) -> str:
    """deep_dive 要素の指示を、モードに応じて切り替える。"""
    mode = cadence["mode"]

    if mode == "weekly_review":
        return f"""【deep_dive — 週次総括モード ({cadence['label']})】
...
"""
    ...
    if mode == "monthly_review":
        return f"""【deep_dive — 月次総括モード ({cadence['label']})】
...
def build_deep_dive_prompt(cadence: dict[str, Any]) -> str:
    universe_prompt = chart_prompt_list()
    deep_dive_block = _build_deep_dive_instructions(cadence)

    return f"""あなたは金融市場のアナリスト兼エディターです。
直近24時間のニュース見出しを元に、マーケット日報の **Deep Dive 解説記事 1 本** を日本語で作成します。

ニュース一覧はインデックス番号 [N] 付きで提供されます。
deep_dive では参照した元記事のインデックス番号を source_index で返してください（元記事URLの紐付けに使います）。
...
def call_deep_dive(
    client: Anthropic, items: list[dict[str, str]], cadence: dict[str, Any]
) -> dict[str, Any]:
    user_msg = (
        f"以下は直近24時間に配信された市場関連ニュースの見出し一覧です "
        f"（計{len(items)}件）。先頭の [N] がインデックス番号です。"
        f"システムプロンプトに従って、deep_dive のみを JSON で返してください。\n\n"
        + build_news_list(items)
    )
    return _call_claude(
        client,
        model=MODEL_OPUS,
        max_tokens=2500,
        system=build_deep_dive_prompt(cadence),
        user_msg=user_msg,
        label="deep_dive",
    )
...
# ============================================================
# Call 3: central_bank_watch + pe_pd_view + real_assets_view (Sonnet 4.6)
# ============================================================

def build_cb_and_alts_prompt(cb_facts: str) -> str:
    cb_section = ""
    if cb_facts:
        cb_section = f"""
...
この情報を **central_bank_watch** で使ってください。

{cb_facts}
"""

    return f"""あなたは金融市場のアナリスト兼エディターです。
直近24時間のニュース見出しを元に、マーケット日報の **中銀ウォッチ + オルタナティブ解説** を日本語で作成します。

ニュース一覧はインデックス番号 [N] 付きで提供されます。
pe_pd_view / real_assets_view では参照した元記事のインデックス番号配列を source_indices で返してください。{cb_section}

【central_bank_watch (4本)】
...
【pe_pd_view (プライベート・エクイティ／プライベート・デット)】
...
def call_cb_and_alts(
    client: Anthropic, items: list[dict[str, str]], cb_facts: str
) -> dict[str, Any]:
    user_msg = (
        f"以下は直近24時間に配信された市場関連ニュースの見出し一覧です "
        f"（計{len(items)}件）。先頭の [N] がインデックス番号です。"
        f"システムプロンプトに従って、central_bank_watch / pe_pd_view / real_assets_view "
        f"の3要素を JSON で返してください。\n\n"
        + build_news_list(items)
    )
    return _call_claude(
        client,
        model=MODEL_SONNET,
        max_tokens=2500,
        system=build_cb_and_alts_prompt(cb_facts),
        user_msg=user_msg,
        label="cb+alts",
    )
...
# ============================================================
# Call 4: funny_stories + economic_chart_of_the_day (Haiku 4.5)
# ============================================================

def build_muse_and_economic_prompt() -> str:
    return """あなたは金融市場のアナリスト兼エディターです。
直近24時間のニュース見出しを元に、マーケット日報の **小話 + 経済指標選定** を日本語で作成します。

ニュース一覧はインデックス番号 [N] 付きで提供されます。
funny_stories ではネタ元がある場合は source_index を付ける、ない場合は null。
...
{
  "funny_stories": [
    { "kind": "皮肉",   "title": "...", "body": "...", "source_index": 7 },
    { "kind": "人間味", "title": "...", "body": "...", "source_index": null },
    { "kind": "観察",   "title": "...", "body": "...", "source_index": 12 }
  ],
  "economic_chart_of_the_day": {
    "series_id": "ICSA",
    "title": "...",
...
def call_muse_and_economic(
    client: Anthropic, items: list[dict[str, str]]
) -> dict[str, Any]:
    user_msg = (
        f"以下は直近24時間に配信された市場関連ニュースの見出し一覧です "
        f"（計{len(items)}件）。先頭の [N] がインデックス番号です。"
        f"システムプロンプトに従って、funny_stories / economic_chart_of_the_day "
        f"の2要素を JSON で返してください。\n\n"
        + build_news_list(items)
    )
    return _call_claude(
        client,
        model=MODEL_HAIKU,
        max_tokens=1500,
        system=build_muse_and_economic_prompt(),
        user_msg=user_msg,
        label="muse+econ",
    )
...
# ============================================================
# Orchestrator: 4 calls を独立して実行 (1つの失敗が全体を落とさない)
# ============================================================

# 各 call が失敗した時の fallback (キーは生成しないか空にする — 下流の attach/normalize は耐性あり)
_FALLBACK_NEWS_AND_CHARTS = {
    "epigraph": {
        "quote": "市場は、語られないことに最もよく語る。",
        "source": "(fallback)",
        "connection": "Claude API の生成失敗。本日は静謐をご堪能ください。",
    },
    "headline_of_the_day": "(コンテンツ生成失敗)",
    "news": [],
    "charts_of_the_day": [],
}

_FALLBACK_DEEP_DIVE = {"deep_dive": None}

_FALLBACK_CB_AND_ALTS = {
    "central_bank_watch": [],
    "pe_pd_view": None,
    "real_assets_view": None,
}

_FALLBACK_MUSE_AND_ECON = {
    "funny_stories": [],
    "economic_chart_of_the_day": None,
}


def run_all_calls(
    items: list[dict[str, str]], cadence: dict[str, Any]
) -> dict[str, Any]:
    """4 つの Claude API 呼び出しを独立して実行し、結果を merge する。

    1 つでも失敗したら fallback で埋めて続行 — 全要素が消える事故を防ぐ。
    """
    ...
    # Call 1: news + charts (Opus 4.7) — 最重要、ここの失敗は痛い
    try:
        result.update(call_news_and_charts(client, items))
    except Exception as e:
        log_warn(f"news+charts call failed: {e}")
        result.update(_FALLBACK_NEWS_AND_CHARTS)

    # Call 2: deep_dive (Opus 4.7)
    try:
        result.update(call_deep_dive(client, items, cadence))
    except Exception as e:
        log_warn(f"deep_dive call failed: {e}")
        result.update(_FALLBACK_DEEP_DIVE)

    # Call 3: cb + alts (Sonnet 4.6)
    try:
        result.update(call_cb_and_alts(client, items, cb_facts))
    except Exception as e:
        log_warn(f"cb+alts call failed: {e}")
        result.update(_FALLBACK_CB_AND_ALTS)

    # Call 4: muse + economic (Haiku 4.5)
    try:
        result.update(call_muse_and_economic(client, items))
    except Exception as e:
        log_warn(f"muse+econ call failed: {e}")
        result.update(_FALLBACK_MUSE_AND_ECON)

    return result


# ============================================================
# Post-processing (既存 v7 から踏襲、無修正)
# ============================================================

def attach_source_urls(
    payload: dict[str, Any], items: list[dict[str, str]]
) -> dict[str, Any]:
    def _get(idx):
        if idx is None:
            return None, None
...
def main() -> None:
    ...
    log_info(f"Cadence: {cadence['mode']} ({cadence['label']})")

    items = fetch_rss_items()

    if not items:
        log_warn("No items fetched. Writing fallback.")
        payload = {
        ...
        # v13.2: 4 つの Claude API 呼び出しを実行 (失敗の独立性あり)
        summary = run_all_calls(items, cadence)
