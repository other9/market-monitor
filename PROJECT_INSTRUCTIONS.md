...
| バージョン | 主な成果 |
|---|---|
| **v13.4.0** | Ruff + ESLint v9 + Prettier + Dependabot + CI ワークフロー新設 (`ci.yml`) |
...
| Python カバレッジ (Codecov) | `pytest --cov=scripts` | **24%** ベースライン (主要 2 スクリプト 84-92%) |
...
ROADMAP の Phase 2 = v16.0:
- `scripts/fetch_edinet_filings.py` — EDINET 大量保有報告書 + 公開買付届出書
- `scripts/fetch_tdnet_disclosures.py` — TDnet 適時開示 (自己株式取得 / 業績修正 / TOB / MBO / 株主提案 等)
- 新セクション「**10. 日本株ウォッチ**」
- アクティビスト watchlist (シティ系 / ダルトン / Strategic Capital / 3D / エフィッシモ / オアシス / 村上系)
- UI 表現は「最近の開示」「該当企業」の中立的ラベル (DECISION v13.4-plan-03)

詳細は [`ROADMAP.md`](ROADMAP.md) を参照。

### Phase 1 で日常運用に追加されたコマンド
...
pytest tests/ -v                                # 35 件
pytest tests/ --cov=scripts --cov-report=term   # カバレッジ計測
...
- **Codecov のカバレッジ向上余地**: v13.5 時点で TOTAL 24%。`fetch_news.py` / `fetch_central_banks.py` / `fetch_economic_chart.py` / `fetch_featured_charts.py` / `fetch_listed_alts.py` / `fetch_valuations.py` / `archive_data.py` が 0%。v13.5.x で段階追加の余地
