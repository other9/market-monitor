...
- バージョン番号は連番で増やす (現行 v13.3)
- 軽い修正は v13.3.1 のような小数刻みも可

## 現状のステータス
最新バージョン: **v13.3** — 土台拡充フェーズ完了

v13 は「土台拡充フェーズ」(機能追加と分離して保守性・コスト効率・運用品質を高める)。
全体方針は [`ROADMAP.md`](ROADMAP.md) を参照。**v13.3 をもって v13 系統は全て完了**、
これ以降は機能追加 (v14+) のフェーズに移行可能。

### v13.3 で完了したこと (common.py 全面活用)
- **9 つの Python スクリプトを `scripts/common.py` に乗せ替え**:
  - FRED 直叩き 5 ファイル → `fred_observations()` ラッパに置換 (戻り値型は呼び出し側で整形)
  - inline `extract_close_series` 2 ファイル → import に統一
  - 全ファイルの `print(f"[OK]...")` → `log_ok/log_warn/log_skip/log_info` に統一
  - `datetime.now(timezone(timedelta(hours=9)))` → `jst_now()` / `jst_today_iso()` に集約
- **検証**: AST parse 10/10 OK、namespace package mock import 10/10 OK、`pytest tests/test_common.py` 18/18 PASS

### v13.2 で完了したこと (Claude API 4 分割)
- **`fetch_news.py` の Claude API 呼び出しを 4 つに分割** (失敗の独立性 + コスト効率):
...
- 各 call が独立して try/except、1 つ失敗しても他は生成される (`_FALLBACK_*` 定数)
- 出力 `data/news.json` の JSON schema は v7 と完全に同一 (フロント影響なし)

### v13.1.3 で完了したこと (フロント分割 Phase 2)
- **残り 10 セクションを `src/components/sections/` に切り出し**:
  - `FeaturedChartsSection` / `MarketTableSection` / `SectorHeatmapSection` / `MacroBarometerSection` / `FundingVolSection` / `ValuationsSection` / `CentralBanksSection` / `IndicatorChartsSection` / `NewsSection` / `ListedAltsSection`
- **`MarketMonitor.jsx` の最終形**: 1513 行 → **134 行** (-91%) の薄いオーケストレータに収束
  - URL 定数 + `useEffect` データ取得 + 16 セクションを props 経由で並べる JSX のみ
- **依存方向ルール** (DECISION v13.1-03) を grep で確認、違反ゼロ
- **`SectorHeatmapSection`** だけ内部 `useState` を持つ (期間タブの状態、ローカル UI 状態)

### v13.1.0 / v13.1.1 / v13.1.2 で導入された土台 (継続)
...
- **`src/components/common/`** — Pct / Signed / MiniChart / StaleDataWarning + barrel
- **`src/components/sections/`** — Phase 1 (Masthead/EconomicChart/DeepDive/AlternativesSpotlight/MarketMuse/Footer) を v13.1.2 で切り出し済み

### v13.0 で導入された土台 (継続)
- **`scripts/common.py`** — FRED API client / yfinance MultiIndex 吸収 / 共通ロガー / 日付ヘルパー (v13.3 で全 fetch スクリプトから利用開始)
- **`scripts/take_snapshot.sh`** — 改修確認用 zip を 1 コマンド化
- **`tests/`** — pytest 導入、18 テスト
...
- セクション 9 つ (アラビア数字)
- ブルー×グリーン基調
- Claude モデル: 役割別に Opus 4.7 / Sonnet 4.6 / Haiku 4.5 を使い分け (v13.2 〜)
...
## 次回以降の開発で気を付けたいこと
- v13 系は完了したので、次は **v14 (archive 閲覧 UI)** または **v14.x (yfinance fallback)** が候補 (ROADMAP 参照)
- 新規データソース追加時は `data/*.json` のスキーマ変更を JSX 側 (`src/components/sections/`) でも対応
...
- **新規 fetch スクリプト追加時は最初から `scripts/common.py` を使う** (DECISION v13.3-01..05 参照)
