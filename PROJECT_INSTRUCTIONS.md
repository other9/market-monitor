...
## 現状のステータス
最新バージョン: **v13.4.2** — v13.4 (Phase 1 前半) 完了 ✅

v13 系統 (土台拡充) は v13.3 で完了し、Phase 1 (v13.4) を v13.4.0/.1/.2 の 3 段階で完了。
次は **v13.5 (監視・観測レイヤ)** または **v16.0 (Japan Equities Layer)** に着手するフェーズに入る。
全体方針は [`ROADMAP.md`](ROADMAP.md) を、決定経緯は [`DECISIONS.md`](DECISIONS.md) を、運用は [`docs/RUNBOOK.md`](docs/RUNBOOK.md) を参照。
...
### v13.4 完了サマリ (v13.4.0 + v13.4.1 + v13.4.2)

**v13.4.0** (linter インフラ):
- ✅ Ruff (Python lint) / ESLint v9 flat config (JS lint) / Prettier (config のみ)
- ✅ Dependabot (npm/pip/actions の週次 minor/patch)
- ✅ CI ワークフロー新設 (`.github/workflows/ci.yml`)
- ✅ daily-update.yml に Ruff lint ステップ追加

**v13.4.1** (共通 component + テスト + 型強化):
- ✅ 共通 component 3 個 (`SectionHeader` / `GroupHeader` / `ExternalLink`、19 箇所置換)
- ✅ Vitest スナップショット 16 件
- ✅ `scripts/common.py` の mypy strict 化
- ✅ B023 default-arg pattern 修正

**v13.4.2** (統合テスト + 運用ドキュメント + ブランチ保護):
- ✅ Python 統合テスト 17 件追加 (計 35 件構成)
- ✅ `docs/RUNBOOK.md` 新設 (6 セクション + 過去インシデント 7 件)
- ✅ `.github/CODEOWNERS` 新設
- ✅ `docs/BRANCH_PROTECTION.md` 新設 (GitHub UI 設定手順、bypass 許可つき)

### Quality gates 一覧 (v13.4 完了時点)

| 種類 | コマンド | 件数 / 結果 |
|---|---|---|
| Python lint (Ruff) | `ruff check .` | 0 issues |
| Python 型 (mypy strict) | `mypy` | 0 issues (`scripts/common.py` のみ対象) |
| Python テスト | `pytest tests/` | **35 件 PASS** |
| JS lint (ESLint) | `npm run lint` | 0 issues |
| JS スナップショット (Vitest) | `npm run test:ci` | **15 件 PASS** |
| Vite build | `npm run build` | OK |

### 次の一歩 — v13.5 or v16.0

パターン C (交互前進) に従えば、Phase 1 の後半 v13.5 に進む選択肢と、Phase 2 の v16.0 に飛ぶ選択肢がある。
詳細は `ROADMAP.md` を参照。

### v13.4 で日常運用に追加されたコマンド

```bash
# Python lint + 型チェック + テスト
ruff check .                                    # lint チェック
ruff check . --fix                              # 自動修正
mypy                                            # 型チェック (scripts/common.py)
pytest tests/ -v                                # 35 件
...
## 次回以降の開発で気を付けたいこと
- 現在地は **v13.4 (Phase 1 前半) 完了、v13.5 (監視) または v16.0 (Japan Equities) に着手するフェーズ** の状態
...
- **障害時は最初に [`docs/RUNBOOK.md`](docs/RUNBOOK.md) を見る** (v13.4.2 で新設、6 セクション構成、過去インシデント 7 件記録)
- **ブランチ保護**: GitHub UI 側の設定手順は [`docs/BRANCH_PROTECTION.md`](docs/BRANCH_PROTECTION.md) を参照。kk アカウントと `github-actions[bot]` は bypass 許可 (zip 直 push + 自動 commit を維持)
- **共通 component の使い方** (v13.4.1 で抽出): セクション見出しは `<SectionHeader>`、グループ見出しは `<GroupHeader>`、外部リンクは `<ExternalLink>` を使う (生の `<a target="_blank">` は禁止)
- **統合テスト追加時** (v13.4.2 で 17 件追加): `tests/test_fetch_*.py` の mock パターンを参考に。`yf.download` / `fred_observations` を mock、`OUTPUT_PATH` を `tmp_path` に差し替え
- **Vitest スナップショット**: 新規セクション追加時は `src/__tests__/sections.test.jsx` に 1 件追加。意図的 UI 変更後は `npm run test:ci -- -u`
- **mypy 段階展開**: 新規 fetch スクリプト追加時は最初から型注釈、安定したら `pyproject.toml` の `[tool.mypy] files = [...]` に追加
