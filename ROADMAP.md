...
最終更新: 2026-05-12 (v13.5.1 — Sentry 取り下げ、Phase 1 完了は維持)
...
| **エラー監視** | 未導入 | v13.5 で導入試みたが取り下げ (DECISION v13.5.1-01)、ErrorBoundary fallback で代替 |
...
- [x] ~~**Sentry 連携**~~ → **v13.5.1 で取り下げ** (`src/ErrorBoundary.jsx` 経由の vanilla React fallback のみで運用、DECISION v13.5.1-01)
  - 取り下げ理由: Sentry の signup フローが 14 日 Business trial 強制 enroll で月コスト 0 円維持と相性が悪い
  - 代替: ErrorBoundary fallback UI + Stale Data 警告 + GitHub Actions メール + 朝の目視
...
- [x] **`docs/MONITORING.md`** 新設 (v13.5.1 で 3 サービスに縮小)

### 期待効果

- React render エラーの fallback UI 表示 (ErrorBoundary、Sentry 取り下げ後も維持)
- バンドルサイズ肥大化など定量メトリクスでの回帰検知 (Lighthouse)
- テスト不足の客観的可視化 (Codecov)
- 大型変更の merge 前確認手段 (PR preview)

### 月額コスト追加: 0 円 (全て free tier 内、Sentry 取り下げにより friction も 0)
...
| **有料 SaaS** | 月コスト 0 円維持を方針とする。Codecov / Cloudflare は全て free tier、Sentry は v13.5.1 で取り下げ |
