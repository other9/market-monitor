...
| **v13.4.1** | 共通 component 抽出 + Vitest スナップショット + mypy strict + B023 修正 | 未着手 |
...
#### v13.4.1 で実装予定
- [ ] **共通 component 抽出** (`@/components/common` に 3-5 個)
  - 色付き数値セル / カードレイアウト / テーブルヘッダなど、複数セクションで使い回せるパターンを抽出
- [ ] **Vitest スナップショットテスト** 導入 (5-10 件)
  - 主要セクションの render 結果を fixture data でスナップショット化
- [ ] **`scripts/common.py` の mypy strict 化**
- [ ] **B023 の default-arg pattern 修正** (`fetch_listed_alts.py` の `_at` / `_at_date`)、修正後に pyproject.toml の ignore から外す
