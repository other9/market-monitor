...
- **現行バージョン**: v13.4.0 (Linter/Formatter + Dependabot + CI 分離 完了、v13.4.1 着手予定)
...
pip install ruff                                # v13.4.0 で導入された linter
...
PR を作ると `.github/workflows/ci.yml` で Ruff + ESLint + pytest + Vite build が自動実行される。
main 直 push 時は `daily-update.yml` の冒頭で Ruff + pytest が走る。
