...
- **現行バージョン**: v13.4.1 (共通 component + Vitest + mypy strict + B023 修正 完了、v13.4.2 着手予定)
...
```bash
# Python 側
pip install -r requirements.txt                 # ruff / mypy / stubs も含む (v13.4.0/.1 で追加)
FRED_API_KEY=xxx python scripts/fetch_market_data.py

# Python lint + 型チェック + テスト
ruff check .                                    # lint チェック
ruff check . --fix                              # 自動修正
mypy                                            # 型チェック (scripts/common.py のみ、v13.4.1)
pytest tests/ -v
...
# JS lint + テスト + 整形 (v13.4.0/.1)
npm run lint                                    # ESLint チェック
npm run lint:fix                                # ESLint 自動修正
npm run test                                    # Vitest watch モード (v13.4.1)
npm run test:ci                                 # Vitest 1 回実行 (CI 用)
npm run test:ui                                 # Vitest ブラウザ UI
npm run format                                  # Prettier で整形 (任意、CI では check しない)
```

`http://localhost:5173/` で確認できる。

PR を作ると `.github/workflows/ci.yml` で **Ruff + mypy + pytest + ESLint + Vitest + Vite build** が自動実行される。
main 直 push 時は `daily-update.yml` の冒頭で Ruff + mypy + pytest が走る。
...
> v13.4 以降、このトラブルシュート章は `docs/RUNBOOK.md` に切り出され、より詳細化される予定。
